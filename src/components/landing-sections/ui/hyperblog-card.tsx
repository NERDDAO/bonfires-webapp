...
function HyperBlogCardSkeleton({
  ...
  return (
    <div
      className={cn(
        "rounded-3xl w-full flex flex-col p-7.5 bg-[#FFFFFF05] border-[0.78px] border-[#333333] animate-pulse",
        className
      )}
    >
      {/* Title: matches text-xl line-height (1.75rem) */}
      <div className="h-7 w-3/4 bg-[#FFFFFF15] rounded" />
      {/* Description: matches line-clamp-2 text-sm (2 lines) */}
      <div className="h-10 w-full bg-[#FFFFFF10] rounded mt-2" />
      <div className="mt-2 flex gap-2 flex-wrap items-center">
        <span className="h-6 w-24 bg-[#FFFFFF15] rounded-full" />
      </div>
      <div
        className={cn(
          "mt-4 w-full bg-[#FFFFFF10] rounded-lg border-[0.78px] border-[#333333]",
          variant === "featured" ? "h-64" : "h-32"
        )}
      />
      {/* Stats row: matches icon height 18px + alignment */}
      <div className="mt-4 flex gap-4 items-center">
        {[1, 2, 3].map((i) => (
          <div key={i} className="h-[18px] w-12 bg-[#FFFFFF10] rounded" />
        ))}
      </div>
    </div>
  );
...
export default function HyperBlogCard({
  ...
  return (
    <div
      className={cn(
        "rounded-3xl w-full flex flex-col p-7.5 bg-[#FFFFFF05] hover:bg-[#FFFFFF0A] cursor-pointer transition-colors duration-300 border-[0.78px] border-[#333333]",
        className
      )}
    >
      <div className="font-bold text-xl capitalize">{title}</div>
      <div className="text-[#A9A9A9] text-sm mt-2 mb-2 line-clamp-2">
        {description}
      </div>
      <div className="flex gap-2 flex-wrap mt-auto">
        <span className="font-bold text-xs rounded-full px-3 py-1 bg-dark-s-700 text-white whitespace-nowrap">
          {formattedAuthor}
        </span>
        {[
          formattedTimestamp,
          ...
          <span
            key={item}
            className="text-xs rounded-full px-3 py-1 text-white border border-[#646464]/50 whitespace-nowrap"
          >
            {item}
          </span>
        ))}
